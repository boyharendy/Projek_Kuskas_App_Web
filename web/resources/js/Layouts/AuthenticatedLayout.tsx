...
export default function Authenticated({
    ...
    const renderAvatar = (size: 'sm' | 'md' = 'sm') => {
        const displayName = user?.name || 'K';
        const firstLetter = displayName.trim().charAt(0).toUpperCase() || 'K';
        const avatarUrl = user?.avatar_url;
        
        const sizeClasses = size === 'sm' ? 'w-8 h-8 text-xs' : 'w-10 h-10 text-sm';
        const imgSizeClasses = size === 'sm' ? 'w-8 h-8' : 'w-10 h-10';
        ...
        if (avatarUrl && avatarUrl.length > 5) {
            ...
            return (
                <img 
                    src={src} 
                    alt={displayName} 
                    className={`${imgSizeClasses} rounded-full object-cover border border-white/10`}
                />
            );
        }
        ...
        return (
            <div className={`${sizeClasses} rounded-full bg-gradient-to-br ${gradientClass} flex items-center justify-center font-bold text-white shadow-md border border-white/10`}>
                {firstLetter}
            </div>
        );
    };
